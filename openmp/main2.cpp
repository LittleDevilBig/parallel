...
void gauss_sse_elimination_omp(int thread_num)
{
    __m128 t1, t2, t3, t4;
    for(int k = 0; k < n; k++)
    {
...
void test_gauss_omp(int thread_num = 4)
{
    n = maxN;
    init(n);
    ...
    {
        gauss_sse_elimination_omp(thread_num);
    }
    QueryPerformanceCounter((LARGE_INTEGER *)&tail);
    cout << n << ":\t" << (tail - head) * 1000.0 / freq << "\tms" << endl;
}

void gauss_sse_elimination(int n, float a[][maxN], float d[maxN], float x[maxN])
{
    __m128 t1, t2, t3, t4;
    for(int k = 0; k < n; k++)
    {
...
void test_gauss(void (*func)(int n, float a[][maxN], float d[maxN], float x[maxN]))
{
    int r = maxN;
    init(r);
    QueryPerformanceFrequency((LARGE_INTEGER *)&freq);
...
int main()
{
    test_gauss(gauss_sse_elimination);
    test_gauss_omp(10);
    return 0;
}
